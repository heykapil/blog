...
export default function NowPlaying() {
  ...
  return (
    <div className="flex items-center px-3 xl:px-6 py-2">
      <svg
        fill="#1DB954"
      ...
      <div className="ml-2 inline-flex truncate">
        {data?.songUrl ? (
          ...
          <p className="text-gray-800 dark:text-gray-300 font-medium">Not Listening</p>
        )}
        <span className="mx-2 text-gray-800 dark:text-gray-300">{' – '}</span>
        <p className="text-gray-800 dark:text-gray-300 max-w-max truncate">{data?.artist ?? 'Spotify'}</p>
      </div>
    </div>
  )
}